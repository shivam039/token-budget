...
> `token-budget-claude`) and framework adapters (`token-budget-langchain`,
> `token-budget-vercel-ai`, `token-budget-anthropic`, `token-budget-openai`)
> are on the roadmap as separate, thin peer packages — not part of core, and
> not yet published. Core ships first so it can stabilize before adapters
> build on top of it.
...
- **Framework adapters**: `token-budget-langchain`, `token-budget-vercel-ai`,
  `token-budget-anthropic`, `token-budget-openai`.
