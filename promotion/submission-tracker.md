...
| DevHunt | Dev-tool launch site | devhunt.org | DRAFT READY | 2026-08-31 | **Yes, free** | MEDIUM-HIGH | Confirmed free; explicitly built as "Product Hunt for developer tools" — best taxonomy fit of the whole directory list. Still unreachable from this sandbox (egress-blocked, re-confirmed 2026-08-31) and no browser tool exists here — needs a human to sign in (Google/GitHub/email) and submit. Copy-paste-ready package in `devtool-directory-submissions.md`. |
| DevPages | Dev-tool directory | devpages.io | DRAFT READY | 2026-08-31 | **Yes, free** | MEDIUM | Team-reviewed, free, no paid tier found. Site unreachable from this sandbox (egress-blocked) and no browser tool exists here — needs a human to submit via `devpages.io/submit-a-tool`. Copy-paste-ready package in `devtool-directory-submissions.md`. |
| DevTool.io | Dev-tool directory | devtool.io | DRAFT READY | 2026-08-31 | **Yes, free** (no paid tier found) | MEDIUM | Curated directory (frontend/backend/DevOps/AI-ML/etc.), "Submit a Tool" appears in nav/category pages. Site unreachable from this sandbox (egress-blocked) and no browser tool exists here — needs a human to submit. Copy-paste-ready package in `devtool-directory-submissions.md`. |
| jsDelivr | Package CDN | jsdelivr.com | **COMPLETED** | 2026-08-31 | Free, automatic | LOW-MEDIUM | No submission exists or is needed — jsDelivr auto-serves every public npm package the instant it's published. Every `token-budget` package is already live at `cdn.jsdelivr.net/npm/@shivam.dixit/<package>`. Confirmed via jsDelivr's own documentation. |
| daily.dev | Content aggregator | daily.dev | **NOT RELEVANT** | 2026-08-31 | Free (for eligible sources) | LOW | Checked directly via daily.dev's own docs: "Source" submission requires an eligible blog/publication RSS feed — explicitly excludes personal/corporate blogs, and a GitHub repo isn't a publication at all. Not a mechanistic fit; not forced. A daily.dev Squad link-post is the closer analog but is the same action as the existing Reddit/HN drafts, needs a human account, and the site is also egress-blocked from this sandbox. |
